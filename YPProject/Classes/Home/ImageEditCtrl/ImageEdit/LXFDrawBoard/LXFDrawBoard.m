...
- (void)alterDescLabel {
    ...
    if (!self.curDescLabel) {
        UILabel *descLabel = [[UILabel alloc] initWithFrame:CGRectMake(self.startPoint.x + 10, self.startPoint.y + 10, screenWidth - 30, screenHeight - 30)];
        ...
        [descLabel.layer setBorderWidth:2];
        [descLabel.layer setBorderColor:HEXColor(0x999999).CGColor];
    }
    self.curDescLabel.textColor = self.style.textColor;
    self.curDescLabel.font      = [UIFont systemFontOfSize:self.style.textSize];
    
    self.curDescLabel.text = desc;
    CGSize size = [self.curDescLabel sizeThatFits:CGSizeMake(screenWidth * 0.8, MAXFLOAT)];
    CGRect curDescLabelFrame = self.curDescLabel.frame;
    self.curDescLabel.frame = CGRectMake(curDescLabelFrame.origin.x, curDescLabelFrame.origin.y, size.width + 10, size.height + 15);
}
...
- (void)descLabelTap:(UITapGestureRecognizer *)tap {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(LXFDrawBoard:clickDescLabel:)]) {
        ...
        [UIAlertView bk_showAlertViewWithTitle:@"提示" message:@"删除还是编辑" cancelButtonTitle:@"删除" otherButtonTitles:@[@"编辑"] handler:^(UIAlertView *alertView, NSInteger buttonIndex) {
            if (buttonIndex == 1) {
...
- (void)descLabelPan:(UIPanGestureRecognizer *)pan {
    ...
    [self.closeButton mas_remakeConstraints:^(MASConstraintMaker *make) {
        make.bottom.mas_equalTo(self.curDescLabel.mas_top).offset(10);
        make.left.mas_equalTo(self.curDescLabel.mas_right);
...
- (void)drawImage {
    ...
    CGContextSetLineWidth(ctx, self.style.lineWidth + 2);
...
- (LXFDrawBoardStyle *)style {
    if (!_style) {
        _style = [LXFDrawBoardStyle shareManager];
    }
    return _style;
}
...
{
    [self.descLabelArr enumerateObjectsUsingBlock:^(UILabel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        [obj.layer setBorderWidth:0];
        [obj.layer setBorderColor:HEXColor(0x999999).CGColor];
    }];
    
    BOOL closeHiddent = self.closeButton.hidden;
    self.closeButton.hidden = YES;
    ...
    self.closeButton.hidden = closeHiddent;
...
-(UIButton *)closeButton {
    ...
    if (_closeButton == nil) {
        ...
        [_closeButton setTitleColor:HEXColor(0x000000) forState:(UIControlStateNormal)];
...
- (BOOL)isEdit {
    ...
    return self.doManager.isEdit || self.descLabelArr.count;
}

- (UIImage *)editImage {
    
    if (self.isEdit) {
        return [self snapsHotView:self];
    }
    return nil;
}
