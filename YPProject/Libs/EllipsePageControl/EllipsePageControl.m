...
#define SELECTED_ITEM_SCALE 1.5f
...
-(void)createPointView{
    ...
    CGFloat startX=0;
    CGFloat startY=0;
    CGFloat mainWidth=_numberOfPages*(_controlSize.width+_controlSpacing);
    if(self.frame.size.width<mainWidth){
        startX=0;
    }else{
        startX=(self.frame.size.width-mainWidth)/2;
    }
    if(self.frame.size.height<_controlSize.height){
        startY=0;
    }else{
        startY=(self.frame.size.height-_controlSize.height)/2;
    }
    //动态创建点
    for (int page=0; page<_numberOfPages; page++) {
        if(page==_currentPage){
            UIView *currPointView=[[UIView alloc]initWithFrame:CGRectMake(startX, startY, _controlSize.width*SELECTED_ITEM_SCALE, _controlSize.height)];
            currPointView.layer.cornerRadius=_controlSize.height/2;
            ...
            UIView *otherPointView=[[UIView alloc]initWithFrame:CGRectMake(startX, startY, _controlSize.width, _controlSize.height)];
            otherPointView.backgroundColor=_otherColor;
            otherPointView.tag=page+1000;
            otherPointView.layer.cornerRadius=_controlSize.height/2;
...
-(void)exchangeCurrentView:(NSInteger)old new:(NSInteger)new
{
    ...
    [UIView animateWithDuration:0.3 animations:^{
        ...
        if(new<old)
            lx+=_controlSize.width;
        oldSelect.frame=CGRectMake(lx, mpSelect.origin.y, _controlSize.width, _controlSize.height);
        
        CGFloat mx=newTemp.origin.x;
        if(new>old)
            mx-=_controlSize.width;
        newSeltect.frame=CGRectMake(mx, newTemp.origin.y, _controlSize.width*SELECTED_ITEM_SCALE, _controlSize.height);
        ...
        if(new-old>1)
        {
            for(NSInteger t=old+1;t<new;t++)
            {
                ...
                ms.frame=CGRectMake(ms.frame.origin.x-_controlSize.width, ms.frame.origin.y, _controlSize.width, _controlSize.height);
            }
        }
        // 右边选中到左边的时候 越过点击
        if(new-old<-1)
