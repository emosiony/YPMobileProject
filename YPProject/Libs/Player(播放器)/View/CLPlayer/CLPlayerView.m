...
- (CLPlayerMaskView *) maskView{
    if (_maskView == nil){
        ...
        _backButton = _maskView.backButton;
        //        _shutDownButton = _maskView.shutDownButton;
        //        _shareButton = _maskView.shareButton;
        _maskView.topToolBar.hidden = _isHiddenTopBar;
...
-(void)setIsfillMode:(BOOL)isfillMode {
    _isfillMode = isfillMode;
    self.maskView.showFullButton = _isfillMode;
}
...
-(void)setIsHiddenTopBar:(BOOL)isHiddenTopBar {
    _isHiddenTopBar = isHiddenTopBar;
    self.maskView.topToolBar.hidden = _isHiddenTopBar;
}
...
-(void)setProgressValue:(double)progressValue {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if (self.maskView.activity.isAnimating) [self.maskView.activity stopAnimating];
    });
...
-(void)cl_playButtonAction:(UIButton *)button{
    if (button.selected == NO){
        ...
        _clickPalyBlock ? _clickPalyBlock() : nil;
        if ([self.maskView.currentTimeLabel.text isEqualToString:self.maskView.totalTimeLabel.text]) {
            [self resetPlay];
        } else {
            [self playVideo];
        }
...
- (void)appDidEnterBackground:(NSNotification *)note{
    //将要挂起，停止播放
    if (self.state == CLPlayerStateBuffering ||
        self.state == CLPlayerStatePlaying) {
        [self pausePlay];
    }
}
